...
name_of_file <- "CVD_read_codes_final.xlsx"
...
# Do Some Tidying... Remove Blank Rows
...
#to lower class
df_cleaned$READ_TYPE <- tolower(df_cleaned$READ_TYPE)
...
tryCatch(
  expr = {
    print("Saving")
    sqlSave(sql, df_cleaned, "SAILW0800V.IMPORTED_CVD_READCODES", rownames=FALSE)
    print("Saved")
    },
  ...
  error = function(e){
    ...
    sqlDrop(sql, "SAILW0800V.IMPORTED_CVD_READCODES")
    print("Table Dropped")
    print("Resaving")
    sqlSave(sql,df_cleaned, "SAILW0800V.IMPORTED_CVD_READCODES", rownames=FALSE)
    print("Saved")
