...
name_of_file <- "MH_read_codes_final.xlsx"
...
for (sheet in my_excelSheets){
  ...
  df_subset <- df_subset[,c("pre-2010",	"Study Period (2010-2019)", "READ_CODE", "DESCRIPTION", "READ_TYPE")]
...
tryCatch(
  expr = {
    print("Saving")
    sqlSave(sql, df_cleaned, "SAILW0800V.IMPORTED_MH_READCODES", rownames=FALSE)
    print("Saved")
  },
  ...
  error = function(e){
    ...
    sqlDrop(sql, "SAILW0800V.IMPORTED_MH_READCODES")
    print("Table Dropped")
    print("Resaving")
    sqlSave(sql,df_cleaned, "SAILW0800V.IMPORTED_MH_READCODES", rownames=FALSE)
    print("Saved")
