...
name_of_file <- "CVD_icd_codes_final.xlsx"
...
for (sheet in my_excelSheets){
  ...
  df_subset <- df_subset[,c("pre-2010",	"Study Period (2010-2019)",	"DIAG_CD_123",	"DIAG_DESC_123",	"ROW_STS",	
                            "FROM_DT",	"TO_DT",	"AVAIL_FROM_DT",	"DIAG_CD_4",	"DIAG_DESC_4",	"ICD_TYPE")]
...
#to lower class
df_cleaned$ICD_TYPE <- tolower(df_cleaned$ICD_TYPE)
...
tryCatch(
  expr = {
    print("Saving")
    sqlSave(sql, df_cleaned, "SAILW0800V.IMPORTED_CVD_ICDCODES", rownames=FALSE)
    print("Saved")
  },
  ...
  error = function(e){
    ...
    sqlDrop(sql, "SAILW0800V.IMPORTED_CVD_ICDCODES")
    print("Table Dropped")
    print("Resaving")
    sqlSave(sql,df_cleaned, "SAILW0800V.IMPORTED_CVD_ICDCODES", rownames=FALSE)
    print("Saved")
