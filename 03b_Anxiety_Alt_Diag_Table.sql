DROP TABLE SAILW0800V.MHTable_anxiolytics;
DROP TABLE SAILW0800V.MHTable_anxiety_symptoms;

-- Query GP events for any relevant READ_TYPE (anxiolytics and anxiety symptoms)
-- Create two separate tables... one for anxiolytics (MHTable_anxiolytics), one for anxiety symptoms (MHTable_anxiety_symptoms).
...
-- For purposes of checking, columns from both MHTable_anxiolytics and MHTable_anxiety_symptoms are included...

SELECT * FROM SAILW0800V.IMPORTED_MH_READCODES
...
SELECT ALF_PE, WOB, EVENT_CD, EVENT_VAL, EVENT_DT,
	...
	FROM SAIL0800V.WLGP_GP_EVENT_CLEANSED_20220101 gp
		INNER JOIN (
			...
			AND readcodes.READ_TYPE = 'anxiolytics'
			OR readcodes.READ_TYPE = 'anxiety_symptoms';
		
-----------------------------------------------------------------------
-----------------------------------------------------------------------	
------  Create a table based on the above query... 

CREATE TABLE SAILW0800V.MHTable_anxiolytics AS (
SELECT ALF_PE, WOB, EVENT_CD, EVENT_VAL, EVENT_DT,
	(EVENT_DT - 90 DAY) AS EVENT_ST,
	(EVENT_DT + 90 DAY) AS EVENT_END,
	readcodes.READ_TYPE
	FROM SAIL0800V.WLGP_GP_EVENT_CLEANSED_20220101 gp
		INNER JOIN (
			...
			WHERE EVENT_DT > '0001-01-01'
			AND readcodes.READ_TYPE = 'anxiolytics'
			OR readcodes.READ_TYPE = 'anxiety_symptoms') WITH NO DATA;
-----------------------------------------------------------------------
-----------------------------------------------------------------------
		
---   Duplicate Table Structure for antidepressant...
CREATE TABLE SAILW0800V.MHTable_anxiety_symptoms LIKE SAILW0800V.MHTable_anxiolytics;
		
--------------------  Insert query results into both tables	
INSERT INTO SAILW0800V.MHTable_anxiolytics (
SELECT ALF_PE, WOB, EVENT_CD, EVENT_VAL, EVENT_DT,
	(EVENT_DT - 90 DAY) AS EVENT_ST,
	(EVENT_DT + 90 DAY) AS EVENT_END,
	readcodes.READ_TYPE
...
INSERT INTO SAILW0800V.MHTable_anxiety_symptoms (
SELECT ALF_PE, WOB, EVENT_CD, EVENT_VAL, EVENT_DT,
	(EVENT_DT - 90 DAY) AS EVENT_ST,
	(EVENT_DT + 90 DAY) AS EVENT_END,
	readcodes.READ_TYPE
	FROM SAIL0800V.WLGP_GP_EVENT_CLEANSED_20220101 gp
		...
		INNER JOIN (
			...
			ON readcodes.READ_CODE = gp.EVENT_CD
			WHERE gp.EVENT_DT > '0001-01-01'
			AND readcodes.READ_TYPE = 'anxiety_symptoms'
			AND INTEGER(FLOOR(gp.EVENT_DT-gp.WOB)/10000) >= 18)
...
DROP TABLE SAILW0800V.MHTable_Alternative_Anxiety_Diagnosis;

--------------------  Create table based on query and insert into the results thereof.

SELECT * FROM SAILW0800V.MHTable_Alternative_Anxiety_Diagnosis;

CREATE TABLE SAILW0800V.MHTable_Alternative_Anxiety_Diagnosis AS (
SELECT DR1.ALF_PE, 
	DR1.WOB, 
	...
	FROM SAILW0800V.MHTable_anxiolytics DR1 
		INNER JOIN (SELECT * FROM SAILW0800V.MHTable_anxiety_symptoms) AS DR2
...
INSERT INTO SAILW0800V.MHTable_Alternative_Anxiety_Diagnosis (
SELECT DR1.ALF_PE, 
	DR1.WOB, 
	DR1.EVENT_CD AS ANTIDEPRESSANT_CD, 
	DR1.EVENT_DT AS ANTIDEPRESSANT_DT, 
