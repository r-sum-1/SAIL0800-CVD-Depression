...
----------------------------------------------------
DROP TABLE SAILW0800V.COHORT_BASE_TABLE;
	
CREATE TABLE SAILW0800V.COHORT_BASE_TABLE (
	ALF_PE					BIGINT,
...
INSERT INTO SAILW0800V.COHORT_BASE_TABLE (
...
FROM SAILW0800V.COHORT_ENTRY ch
		...
		WHERE EARLIEST_DATE BETWEEN ENTER_COHORT AND EXIT_COHORT)	;	
...
INSERT INTO SAILW0800V.COHORT_BASE_TABLE (
SELECT DISTINCT ch.ALF_PE, 'dep.alt' AS READ_CODE, 'depression_alt_diag' AS READ_TYPE, aad.EARLIEST_DATE, NULL AS EVENT_VAL, 1 AS HOSP_GP
FROM SAILW0800V.COHORT_ENTRY ch
		LEFT JOIN (SELECT ALF_PE, EARLIEST_DATE FROM SAILW0800V.MHTable_Alternative_Diagnosis WHERE NOT EARLIEST_DATE IS NULL) AS aad 
		ON ch.ALF_PE = aad.ALF_PE
		WHERE EARLIEST_DATE BETWEEN ENTER_COHORT AND EXIT_COHORT);
...
LEFT JOIN (
	...
	WHERE pedw.START_DATE BETWEEN ENTER_COHORT AND EXIT_COHORT
		AND NOT readcodes."StudyPeriod20102019" = 0) ;		
